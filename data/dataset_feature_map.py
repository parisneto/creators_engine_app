    ...
    "df_nerdalytics": {
        ...
        "features": {
            ...
            "ss_adult": {
                ...
                "example": 2,
                "use": "group, filter",
                "description": "Google Vision API SafeSearch score for adult content. Mapped to an integer from 0 (Unknown) to 5 (Very Likely) based on LIKELIHOOD_VALUES in config.py.",
            },
            "ss_spoof": {
                "type": "numerical",
                "subtype": "ordinal_score",
                "example": 1,
                "use": "group, filter",
                "description": "Google Vision API SafeSearch score for spoof content. Mapped to an integer from 0 (Unknown) to 5 (Very Likely) based on LIKELIHOOD_VALUES in config.py.",
            },
            "ss_medical": {
                "type": "numerical",
                "subtype": "ordinal_score",
                "example": 0,
                "use": "group, filter",
                "description": "Google Vision API SafeSearch score for medical content. Mapped to an integer from 0 (Unknown) to 5 (Very Likely) based on LIKELIHOOD_VALUES in config.py.",
            },
            "ss_violence": {
                "type": "numerical",
                "subtype": "ordinal_score",
                "example": 3,
                "use": "group, filter",
                "description": "Google Vision API SafeSearch score for violence content. Mapped to an integer from 0 (Unknown) to 5 (Very Likely) based on LIKELIHOOD_VALUES in config.py.",
            },
            "ss_racy": {
                "type": "numerical",
                "subtype": "ordinal_score",
                "example": 0,
                "use": "group, filter",
                "description": "Google Vision API SafeSearch score for racy content. Mapped to an integer from 0 (Unknown) to 5 (Very Likely) based on LIKELIHOOD_VALUES in config.py.",
            },
            "caption": {
                "type": "boolean",
                "subtype": None,
...
def auto_plot_all(df, feature_map):
    ...
    continuous = [
        ...
        if v["type"] == "numerical" and v["subtype"] == "continuous"
    ]
    discrete = [
        k
        for k, v in feature_map.items()
        if v["type"] == "numerical" and v["subtype"] == "discrete"
    ]
...
def print_summary_report(df, feature_map):
    ...
    for t in ["continuous", "discrete", "categorical", "boolean", "datetime", "text"]:
        features = [
            k
            for k, v in feature_map.items()
            if (v["type"] == t or v.get("subtype") == t)
        ]
...
if __name__ == "__main__":
    ...
    if True:  # Set to True to run examples
        ...
        if len(continuous) >= 2:
            sns.jointplot(
                data=df, x=continuous[0], y=continuous[1], kind="scatter"
            ).fig.suptitle(f"Jointplot: {continuous[0]} vs {continuous[1]}")
            plt.tight_layout()
            plt.show()
        ...
        plot_with_confidence_interval(
            df, x="age_in_days", y="like_count", groupby="video_type", ci=95
        )
    ...
    if True:  # Set to True to run examples
        ...
        ordinal = [
            k
            for k, v in feature_map.items()
            if v["type"] == "categorical" and v.get("subtype") == "ordinal"
        ]
        for col in ordinal:
            le = LabelEncoder()
