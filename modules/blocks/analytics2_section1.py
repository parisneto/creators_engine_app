...
def analytics2_section1(df_nerdalytics, df_playlist_full_dedup):
    ...
    with st.popover("🎯 Playlist Filters", use_container_width=True):
        ...
        # Defaults
        date_col = 'video_added_at' if 'video_added_at' in df_enriched.columns else 'playlist_published_at'
        ...
        # Set/reset state
        reset_triggered = st.button('Reset filters') or st.session_state['filters_reset']
        if reset_triggered:
            ...
            selected_channels = st.multiselect("Select channels", channel_options, default=channel_options)
        ...
        filtered_playlists = df_enriched[df_enriched['playlist_channel_title'].isin(selected_channels)]['playlist_title'].dropna().unique().tolist()
        # Only set session state if not already set or invalid
        if st.session_state['selected_playlists'] is None or not set(st.session_state['selected_playlists']).issubset(filtered_playlists):
        ...
        if (
            st.session_state['selected_playlists'] is None or
            not set(st.session_state['selected_playlists']).issubset(filtered_playlists)
        ):
            st.session_state['selected_playlists'] = filtered_playlists.copy()
        ...
        if 'video_type' in df_enriched.columns:
            video_type_options += sorted([v for v in df_enriched['video_type'].dropna().unique() if v not in video_type_options])
        else:
    ...
    with col1:
        st.metric("Total Views", int(df_filtered['view_count'].sum()) if 'view_count' in df_filtered else 'N/A')
        st.metric("Total Likes", int(df_filtered['like_count'].sum()) if 'like_count' in df_filtered else 'N/A')
        st.metric("Total Comments", int(df_filtered['comment_count'].sum()) if 'comment_count' in df_filtered else 'N/A')
    with col2:
        st.metric("Avg Views per Video", round(df_filtered['view_count'].mean(), 1) if 'view_count' in df_filtered else 'N/A')
        st.metric("Avg Views per Playlist", round(df_filtered.groupby('playlist_id')['view_count'].sum().mean(), 1) if 'view_count' in df_filtered and 'playlist_id' in df_filtered else 'N/A')
    with col3:
        st.metric("Unique Videos", df_filtered['video_id'].nunique() if 'video_id' in df_filtered else 'N/A')
        st.metric("Unique Playlists", df_filtered['playlist_id'].nunique() if 'playlist_id' in df_filtered else 'N/A')
    ...
    if 'view_count' in df_filtered:
        playlist_views = df_filtered.groupby(['playlist_title', 'playlist_id'])['view_count'].sum().reset_index()
    ...
    if all(col in df_filtered for col in ['playlist_id', 'view_count', 'like_count', 'comment_count']):
        playlist_eng = df_filtered.groupby('playlist_id').agg({
            'view_count': 'sum',
            'like_count': 'sum',
            'comment_count': 'sum',
            'playlist_title': 'first'
        }).reset_index()
        playlist_eng['likes_per_view'] = playlist_eng['like_count'] / playlist_eng['view_count']
        playlist_eng['comments_per_view'] = playlist_eng['comment_count'] / playlist_eng['view_count']
        ratio_option = st.selectbox("Select ratio to highlight", ['likes_per_view', 'comments_per_view'])
        fig2 = px.scatter(
            playlist_eng,
            x='likes_per_view',
            y='comments_per_view',
    ...
    if 'playlist_item_published_at' in df_filtered and 'view_count' in df_filtered:
        df_filtered['playlist_item_published_at'] = pd.to_datetime(df_filtered['playlist_item_published_at'], errors='coerce')
        df_filtered['age_in_days'] = (datetime.now() - df_filtered['playlist_item_published_at']).dt.days
    ...
    if 'duration_formatted_seconds' in df_filtered and 'view_count' in df_filtered:
        ...
        duration_range = st.slider("Duration range (seconds)", min_value=duration_min, max_value=duration_max, value=(duration_min, duration_max))
        df_dur = df_filtered[(df_filtered['duration_formatted_seconds'] >= duration_range[0]) & (df_filtered['duration_formatted_seconds'] <= duration_range[1])]
    ...
    if 'video_id' in df_filtered and 'playlist_id' in df_filtered:
        ...
        video_playlist = df_filtered[['video_id', 'playlist_id', 'playlist_title']].drop_duplicates()
        # Count how many playlists each video is present in
        video_counts = video_playlist.groupby('video_id').size().reset_index(name='playlist_count')
        ...
        top_playlists = video_playlist['playlist_id'].value_counts().nlargest(15).index.tolist()
        ...
        sankey_links = pairs.groupby(['playlist_title_x', 'playlist_title_y']).size().reset_index(name='count')
        sankey_links = sankey_links[sankey_links['count'] > 1]  # Only show links with >1 shared video
        # Shorten/wrap long labels
        def short_label(label, maxlen=25):
            return label if len(label) <= maxlen else label[:maxlen-3] + '...'
        sankey_links['playlist_title_x'] = sankey_links['playlist_title_x'].apply(lambda x: short_label(str(x)))
        sankey_links['playlist_title_y'] = sankey_links['playlist_title_y'].apply(lambda x: short_label(str(x)))
        labels = list(set(sankey_links['playlist_title_x']).union(set(sankey_links['playlist_title_y'])))
        ...
        st.info("Sankey diagram requires playlist/video overlap logic and view counts. Columns needed: video_id, playlist_id.")
    st.subheader("4.2 Bipartite Network Graph (placeholder)")
    st.info("Bipartite graph requires networkx/pyvis and edge data. Implement if needed.")
    ...
    if 'playlist_item_published_at' in df_filtered and 'view_count' in df_filtered:
        ...
        df_filtered['cum_views'] = df_filtered.groupby('playlist_id')['view_count'].cumsum()
        fig7 = px.line(
            df_filtered,
    ...
    if 'playlist_item_published_at' in df_filtered and 'view_count' in df_filtered:
        df_filtered['cohort_month'] = df_filtered['playlist_item_published_at'].dt.to_period('M')
        df_filtered['days_since_release'] = (df_filtered['playlist_item_published_at'] - df_filtered['playlist_item_published_at'].min()).dt.days
        cohort_group = df_filtered.groupby(['cohort_month', 'days_since_release'])['view_count'].median().reset_index()
        fig8 = px.line(
            cohort_group,
        ...
        st.info("playlist_item_published_at or view_count not found for cohort analysis.")
    ...
    treemap_df = (
        df_filtered
        .dropna(subset=['category_id', 'playlist_title', 'view_count'])
        .copy()
        .astype({
            'category_id':    str,
            'playlist_title': str
    ...
    if not treemap_df.empty and 'category_id' in treemap_df and 'view_count' in treemap_df:
        fig5 = px.treemap(
            treemap_df,
    ...
    if ('default_audio_language' in df_filtered or ss_cols_present) and 'view_count' in df_filtered:
        cols = ['default_audio_language'] + ss_cols_present
        melted = df_filtered.melt(id_vars=['view_count'], value_vars=cols, var_name='feature', value_name='value')
        fig6 = px.bar(
            melted,
            x='feature',
            y='view_count',
        ...
        st.info("default_audio_language or ss_* columns not found. Add these to enable this section.")
    ...
    st.info("Implement: Radar chart of channel metrics. Requires more channel-level data.")
    ...
    clustering_features = [col for col in ['view_count', 'like_count', 'duration_formatted_seconds'] if col in df_filtered]
    if len(clustering_features) >= 2:
        st.info(f"Ready for clustering: features available: {clustering_features}. Implement t-SNE/UMAP for 2D embedding.")
        # Placeholder for clustering implementation
    else:
        st.info("Requires clustering features (view_count, like_count, duration, tags embedding, etc.).")
