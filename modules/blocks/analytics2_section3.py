...
def analytics2_section3(df_nerdalytics, df_playlist_full_dedup):
    ...
    """
    # Step 1: Merge playlist and video analytics (same as section1)
    ...
    df_filtered = df_enriched[df_enriched['playlist_channel_title'].isin(selected_channels)]
    ...
    for channel, group in channel_group:
        ...
        # --- 4. Growth & Velocity ---
        group_dates = pd.to_datetime(group['video_added_at'], errors='coerce').sort_values()
        playlist_lifespan_days = (group_dates.max() - group_dates.min()).days if not group_dates.empty else 0
        playlist_lifespan_months = safe_div(playlist_lifespan_days, 30.44)
        avg_time_between_adds = group_dates.diff().dt.days.mean() if len(group_dates) > 1 else 0
        ...
        tags_series = group['tags'].dropna().apply(lambda x: x if isinstance(x, list) else str(x).split(','))
        n_tags = len(set([tag.strip() for tags in tags_series for tag in tags])) if not tags_series.empty else 0
        # Language mix
        ...
        channel_total_videos = df_nerdalytics[df_nerdalytics['channel_title'] == channel]['video_id'].nunique()
        channel_total_views = df_nerdalytics[df_nerdalytics['channel_title'] == channel]['view_count'].sum()
    ...
    selected_metrics = st.multiselect(
        ...
        default=['avg_playlist_size', 'avg_views_per_video', 'overlap_ratio', 'videos_per_month', 'n_categories'],
    ...
    with st.expander("Show raw channel metrics"):
        df_display = df_radar.set_index('channel')[selected_metrics].rename(columns=metric_labels).copy()
        ...
        if overlap_label in df_display.columns:
            formatters[overlap_label] = lambda x: f"{x*100:,.2f}%" if pd.notnull(x) else ""
